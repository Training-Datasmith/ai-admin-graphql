...
abstract class Base
{
     ...
     */
    public function __construct(\Aimeos\M_Shop\Context_Iface $context, private Registry $registry)
    {
     ...
     */
    protected function aggregate_items(string $domain): \Closure
    {
     ...
     */
    protected function delete_items(string $domain): \Closure
    {
        return function ($root, array $args, $context) use ($domain) {
            $this->access($domain, 'delete');
            \Aimeos\M_Shop::create($this->context(), $domain)->delete($args['id']);
            return $args['id'];
        };
    }
    /**
     ...
     */
    protected function get_item(string $domain): \Closure
    {
        return function ($root, array $args, $context) use ($domain): \Aimeos\M_Shop\Common\Item\Iface {
            $this->access($domain, 'get');
            return $this->filter(\Aimeos\M_Shop::create($this->context(), $domain)->get($args['id'], $args['include']));
        };
    }
    /**
    ...
    protected function find_item(string $domain): \Closure
    {
        return function ($root, array $args, $context) use ($domain): \Aimeos\M_Shop\Common\Item\Iface {
            $this->access($domain, 'get');
            return $this->filter(\Aimeos\M_Shop::create($this->context(), $domain)->find($args['code'], $args['include']));
        };
    }
    /**
     ...
     * @return \Closure Anonymous method returning one item
     */
    protected function find_type_item(string $domain): \Closure
    {
        return function ($root, array $args, $context) use ($domain): \Aimeos\M_Shop\Common\Item\Iface {
            $this->access($domain, 'get');
            return $this->filter(\Aimeos\M_Shop::create($this->context(), $domain)->find($args['code'], [], $args['domain']));
        };
    }
    /**
    ...
    protected function search_items(string $domain): \Closure
    {
        return function ($root, array $args, $context) use ($domain): array {
            ...
            $filter->add($filter->parse(json_decode($args['filter'], true)));
            $total = 0;
            $items = $this->filters($manager->search($filter, $args['include'], $total)->to_array());
     ...
     * @return \Closure Anonymous method returning one item
     */
    protected function save_item(string $domain): \Closure
    {
        return function ($root, array $args, $context) use ($domain) {
            ...
            $ref = $this->get_refs($entry, $domain);
            $manager = \Aimeos\M_Shop::create($this->context(), $domain);
            ...
            return $manager->save($this->update_item($manager, $item, $entry));
        };
    }
    /**
     ...
     */
    protected function save_items(string $domain): \Closure
    {
        return function ($root, array $args, $context) use ($domain) {
            ...
            foreach ($entries as $entry) {
                $ref = array_merge($ref, $this->get_refs($entry, $domain));
            }
            $map = $manager->search($filter, array_unique($ref));
            $items = [];
            foreach ($entries as $entry) {
                $item = $map->get($entry[$key] ?? null) ?: $manager->create();
                $items[] = $this->update_item($manager, $item, $entry);
            }
     ...
     */
    protected function get_refs(array $entry, string $domain): array
    {
        $ref = array_keys($entry['lists'] ?? []);
        foreach ($entry['lists'] ?? [] as $list_domain => $subentry) {
            foreach ($subentry ?? [] as $sub_item) {
                $ref = array_merge($ref, $this->get_refs($sub_item['item'] ?? [], $list_domain));
            }
        }
